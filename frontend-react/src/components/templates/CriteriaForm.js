...
function CriteriaForm(props) {
    ...
    // Initialize states
    useEffect(() => {
        djangoLookup("GET", "/criteria/", {}, (response, status) => {
            setMyCriteria(response)
        })
        djangoLookup("GET", "/criteria/?action=exclude&user=" + username, {}, (response, status) => {
            setOpenCriteria(response)   
        })
    }, [])

    // Update open criteria after adding another one
    useEffect(() => {
    ...
    // Updates open criteria and field after deleting one
